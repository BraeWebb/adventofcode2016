...
def parse_v2(text):
    ...
    new_string = ''
    for bracket in re.finditer('\((.*?)\)', text):
        start, end = bracket.span(0)
        length, count = (int(x) for x in bracket.group(0).strip('()').split('x'))
        if start >= last_start:
            subtext = parse_v2(text[end:end+length])
            new_string = new_string + text[last_start:start] + subtext*count
            last_end, last_start = end, end + length
    new_string = new_string + text[last_start:]
    return new_string
